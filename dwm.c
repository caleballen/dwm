...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : occ & 1 << i ? SchemeTagsEmpty : SchemeTagsNorm]);
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = g, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + g, m->wy + my, mw - (2*c->bw) - g, h - (2*c->bw), 0);
			if (my + HEIGHT(c) + g < m->wh)
				my += HEIGHT(c) + g;
		} else {
			h = (m->wh - ty) / (n - i) - g;
			resize(c, m->wx + mw + g, m->wy + ty, m->ww - mw - (2*c->bw) - 2*g, h - (2*c->bw), 0);
...
updatesystray(void)
{
	...
	unsigned int sw = tw - lrpad + systrayspacing;
	unsigned int w = 1;
